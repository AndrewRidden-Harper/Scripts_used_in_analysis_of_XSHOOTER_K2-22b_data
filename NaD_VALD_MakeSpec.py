...
def makespec(species,flux=0.01,sigma='instr_res'):
    ...
    fname = path+'VALD_NaD_T2000K.npy' 
    ...
    leftwlim = 5889.95 - 1000
    rightwlim = 5895.92 + 1000
...
#        tempatepixsigma = tempatepixsigma + 1.0        
    ...
    savedir = '%s/NaD_F%.2E_s%f/' % (path,flux,wsigma)    
#    
    if not os.path.exists(savedir):
        os.makedirs(savedir)
    
    np.save('%s/fullmodel_VALD_NaD_T2000K.npy' % (savedir),results_array)    
              ###/ (path ends in a /)
